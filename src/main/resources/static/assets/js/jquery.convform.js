...
ConvState.prototype.answerWith = function(answerText, answerObject) {
    ...
    if(answerText == "TLSearch") {
        flag = false;
    }
    if(answerText == "BaiduSearch"){
        flag = true;
    }
    //以上flag设置会让flag被最后一个baidu or tl决定flag
    if(!flag){
        if(answerText != "TLSearch" ){
            var message = $('<div class="message from">' + answerText + '</div>');
            $("#chat").on("click", "#submit", function () {

                var keyword = answerText;
                $.ajax({
                    url: '/action/tl?keyword=' + keyword,
                    type: "post",
                    success: function (result) {
                        if (result != 0) {
                            var info = $('<div style="font-size: 80%;" class="message to ready">' + result + '</div>');
                            $("#messages").append(info);
                        }else {
                            var info = $('<div class="message to ready" style="color:red;">' + "本站暂无相关内容!~" + '</div>');
                            $("#messages").append(info);
                        }
                    }
                })
    ...
    else if(answerText != "BaiduSearch" && answerText != "TLSearch" && flag) {
        ...
        $("#chat").on("click", "#submit", function () {
            ...
            var keyword = answerText;
            $.ajax({
                url: '/action/baidu?keyword=' + keyword,
                type: "post",
                success: function (result) {
                    if(result == null){
                        ...
                        var info = $('<div class="message to ready">' + result + '</div>');
                        $("#messages").append(info);
                    }else {
                        var info = $('<div class="message to ready" style="color:red;">' + "暂无结果，或者被暂时百度官网限制了爬取数量！请点击按钮关闭后过10s左右再次查询" + '</div>');
                        $("#messages").append(info);
                    }
                }
            })
